...
export class CreateSource {
    ...
    constructor() {
        ...
        this.attributionSelectElement = document.querySelector('select[data-attribution-select]');
        this.attributionRenderTarget = document.getElementById('attribution-render-target');
    }
    ...
    private async renderAttributionPreview() {
        const selectedAttribution = this.attributionSelectElement.selectedOptions[0];
        if (selectedAttribution.value !== "") {
            const renderedAttribution = await this.getAttributionPreview(selectedAttribution.value);
    ...
    private async getAttributionPreview(attributionId: string) {
        try {
            const response = await fetch(`${this.attributionRendererEndpoint}/${attributionId}`);
