...
export class PrintPanel implements SidebarPanel {
  ...
  pdfPageSettings: PDFPageSettings = {
    a2: {
      ...
      titleFontSize: 16,
      subtitleFontSize: 12,
      standaloneLegendTitleFontSize: 20,
      pageWidth: 594,
      pageHeight: 420,
      inlineLegendPortraitMaxWidth: 140,
      inlineLegendLandscapeMaxWidth: 200,
      maxLandscapeTitleLength: 170,
      maxPortraitTitleLength: 130,
      maxLandscapeSubtitleLength: 500,
      maxPortraitSubtitleLength: 350,
      landscapeKeyWrapLimit: 800,
      portraitKeyWrapLimit: 650,
    },
    a3: {
      attributionFontSize: 11,
      titleFontSize: 14,
      subtitleFontSize: 12,
      standaloneLegendTitleFontSize: 18,
      pageWidth: 420,
      pageHeight: 297,
      inlineLegendPortraitMaxWidth: 80,
      inlineLegendLandscapeMaxWidth: 140,
      maxLandscapeTitleLength: 130,
      maxPortraitTitleLength: 90,
      maxLandscapeSubtitleLength: 350,
      maxPortraitSubtitleLength: 240,
      landscapeKeyWrapLimit: 650,
      portraitKeyWrapLimit: 500,
    },
    a4: {
      attributionFontSize: 10,
      titleFontSize: 12,
      subtitleFontSize: 10,
      standaloneLegendTitleFontSize: 16,
      pageWidth: 297,
      pageHeight: 210,
      inlineLegendLandscapeMaxWidth: 80,
      maxLandscapeTitleLength: 100,
      maxPortraitTitleLength: 70,
      maxLandscapeSubtitleLength: 310,
      maxPortraitSubtitleLength: 170,
      landscapeKeyWrapLimit: 430,
      portraitKeyWrapLimit: 800,
    },
    a5: {
      attributionFontSize: 8,
      titleFontSize: 11,
      subtitleFontSize: 10,
      standaloneLegendTitleFontSize: 14,
      pageWidth: 210,
      pageHeight: 148,
      maxLandscapeTitleLength: 70,
      maxPortraitTitleLength: 50,
      maxLandscapeSubtitleLength: 160,
      maxPortraitSubtitleLength: 100,
      landscapeKeyWrapLimit: 700,
      portraitKeyWrapLimit: 700,
    },
  };
  ...
  async init() {
    try {
      this.printConfiguration = await this.getPrintConfiguration();
      ...
      console.error(ex);
      PrintPanel.showPrintSetupError();
    }
  }
  ...
  private updateValidationRules() {
    ...
    const printTitleInput: HTMLInputElement =
      container.querySelector("#gifw-print-title");
    const printSubtitleInput: HTMLElement = container.querySelector(
      "#gifw-print-subtitle",
    );
    const printPageSizeInput: HTMLSelectElement = container.querySelector(
      "#gifw-print-pagesize",
    );
    const printLegendInput: HTMLSelectElement =
      container.querySelector("#gifw-print-legend");
    const printLegendSizeWarning: HTMLDivElement = container.querySelector(
      "#gifw-print-legend-size-warning",
    );
    let pageSize: PageSizeOption = "a4";
    if (
      printPageSizeInput.value.substring(0, 2) === "a5" ||
      printPageSizeInput.value.substring(0, 2) === "a3" ||
      printPageSizeInput.value.substring(0, 2) === "a2"
    ) {
    ...
    const maxTitleLen =
      pageOrientation === "l"
        ? chosenPageSettings.maxLandscapeTitleLength
        : chosenPageSettings.maxPortraitTitleLength;
    const maxSubtitleLen =
      pageOrientation === "l"
        ? chosenPageSettings.maxLandscapeSubtitleLength
        : chosenPageSettings.maxPortraitSubtitleLength;
    ...
    if (pageSize === "a5" || (pageSize === "a4" && pageOrientation === "p")) {
      ...
      (
        printLegendInput.querySelector(
          `option[value="${<LegendPositioningOption>"float-left"}"]`,
        ) as HTMLOptionElement
      ).disabled = true;
      (
        printLegendInput.querySelector(
          `option[value="${<LegendPositioningOption>"pinned-left"}"]`,
        ) as HTMLOptionElement
      ).disabled = true;
  ...
  private attachPrintControls(): void {
    ...
    const printToScaleCheckbox: HTMLInputElement = container.querySelector(
      "#gifw-print-scale-print",
    );
    const printScaleContainer: HTMLElement = container.querySelector(
      "#gifw-print-scale-container",
    );
    ...
    printButton.addEventListener("click", (e) => {
      if (this.validatePrintOptions()) {
        ...
        const err = new CustomError(
          ...
          "Cannot print",
          "Your title or subtitle is too long",
        );
        err.show();
      }
      e.preventDefault();
    ...
    container.addEventListener("gifw-export-cancel", () => {
      this.cancelExport();
    });
  }
  ...
  private async doPrint() {
    ...
    const pageSetting = (
      document.getElementById("gifw-print-pagesize") as HTMLSelectElement
    ).value;
    let pageSize: PageSizeOption = "a4";
    if (
      pageSetting.substring(0, 2) === "a5" ||
      pageSetting.substring(0, 2) === "a3" ||
      pageSetting.substring(0, 2) === "a2"
    ) {
    ...
    const legend = (
      document.getElementById("gifw-print-legend") as HTMLSelectElement
    ).value as LegendPositioningOption;
    ...
    const isScalePrint = (
      document.getElementById("gifw-print-scale-print") as HTMLInputElement
    ).checked;
    const scale = parseInt(
      (document.getElementById("gifw-print-scale") as HTMLSelectElement).value,
    );

    this.abortController = new AbortController();
    this.cancelledByUser = false;

    document.getElementById(map.id).addEventListener(
      "gifw-print-finished",
      (e: CustomEvent) => {
        this.hideLoading();
        ...
        if (e.detail.success !== true) {
          Alert.showPopupError(
            "Your print failed",
            `<p>Your print failed to generate. Try turning off any layers you don't need, or choosing a smaller size or lower quality. If you continue to have problems, please let us know.`,
          );
        } else {
          if (e.detail.keyWasMoved === true) {
            const msg = new CustomError(
              AlertType.Popup,
              AlertSeverity.Info,
    ...
    this.showLoading();
    this.longLoadingTimeout = window.setTimeout(
      () => this.showLongLoadingWarning(),
      this.longLoadingTimeoutLength,
    );
  }
  ...
  private showLoading() {
    const container: HTMLElement = document.querySelector(this.container);
    const printButton: HTMLButtonElement = container.querySelector(
      "#gifw-print-do-print",
    );
    ...
    Helper.addFullScreenLoader(
      ...
      () => {
        ...
        container.dispatchEvent(new Event("gifw-export-cancel"));
      },
    );
  }
  ...
  private showLongLoadingWarning(): void {
    const mapEle = document.getElementById(
      `${this.gifwMapInstance.id}Container`,
    );
    ...
    if (cancelButton) {
      cancelButton.insertAdjacentHTML(
        "afterend",
        `<p class="mt-2">This map is taking a while to generate.</p><p>You can cancel it with the button above and try removing some layers or reducing the quality to speed it up.</p>`,
      );
    }
  }

  private static showPrintSetupError() {
    const errDialog = new CustomError(
      AlertType.Popup,
      AlertSeverity.Danger,
      "Error setting up print panel",
      "<p>There was an error setting up the print panel</p><p>This means the print functionality will not work. Please refresh the page to try again</p>",
    );
    errDialog.show();
