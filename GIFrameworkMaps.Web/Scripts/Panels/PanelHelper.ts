...
export class PanelHelper {
   ...
   */
  public static renderSliderControl(
    layerId: string,
    startingValue: number = 100,
    step: number = 1,
    controlType: "saturation" | "opacity",
    layerType: "layer" | "basemap",
    ...
    toggleMinimumCheckboxLabel.textContent =
      controlType === "saturation" ? "Greyscale" : "Invisible";
    //checkbox container
    ...
    control.addEventListener("input", (e) => {
      ...
      if (controlType === "saturation") {
        if (layerType === "layer") {
          gifwMapInstance.setLayerSaturation(
            gifwMapInstance.getLayerById(layerId) as olLayer<
              Source,
              LayerRenderer<olLayer>
            >,
            value,
          );
        } else {
        ...
        if (layerType === "layer") {
          gifwMapInstance.setLayerOpacity(
            gifwMapInstance.getLayerById(layerId) as olLayer<
              Source,
              LayerRenderer<olLayer>
            >,
