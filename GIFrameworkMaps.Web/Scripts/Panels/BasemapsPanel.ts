...
export class BasemapsPanel implements SidebarPanel {
  ...
  private renderBasemapsPanel(): void {
    const container = document.querySelector(this.container);
    const basemapsListContainer = container.querySelector(
      "#basemaps-list-container",
    );
    /*TODO - Update rendering to only do a full render once, then just update on following renders*/
    basemapsListContainer.innerHTML = "";
    const basemaps = this.gifwMapInstance
      .getLayerGroupOfType(LayerGroupType.Basemap)
      .olLayerGroup.getLayersArray()
      .sort((basemapA, basemapB) => {
        const sortA = this.gifwMapInstance.config.basemaps.filter(
          (b) => b.id === basemapA.get("layerId"),
        )[0].sortOrder;
        const sortB = this.gifwMapInstance.config.basemaps.filter(
          (b) => b.id === basemapB.get("layerId"),
        )[0].sortOrder;
        return sortA - sortB;
      });

    basemaps.forEach((basemap) => {
      const basemapConfiguration = this.gifwMapInstance.config.basemaps.filter(
        (b) => b.id === basemap.get("layerId"),
      )[0];
      basemapsListContainer.append(
        this.renderBasemapTile(basemap, basemapConfiguration),
      );
      basemapsListContainer.append(
        this.renderBasemapMeta(basemap, basemapConfiguration),
      );
    });
  }
   ...
   */
  private renderBasemapTile(
    basemap: olLayer<Source, LayerRenderer<olLayer>>,
    basemapConfiguration: Basemap,
  ): HTMLElement {
    const card = document.createElement("div");
    card.className = `card text-white basemaps-selector ${
      basemap.getVisible() ? "active" : ""
    }`;
    card.style.backgroundImage = `${
      basemapConfiguration.previewImageURL
        ? `url(${basemapConfiguration.previewImageURL})`
        : ""
    }`;
   ...
   */
  private renderBasemapMeta(
    basemap: olLayer<Source, LayerRenderer<olLayer>>,
    basemapConfiguration: Basemap,
  ): HTMLElement {
    const meta = document.createElement("div");
    meta.className = `basemaps-meta border-start border-bottom border-2 d-block ${
      basemap.getVisible() ? "d-block" : "d-none"
    }`;
  ...
  private toggleBasemap(basemapId: string) {
    const basemapSelector = document.getElementById(
      `basemaps-selector-${basemapId}`,
    );
    ...
    if (basemapSelector.classList.contains("active")) {
      ...
      const layerGroup = this.gifwMapInstance.olMap
        .getLayers()
        .getArray()
        .filter((g) => g.get("type") === "base")[0];
      const basemap = layerGroup
        .getLayersArray()
        .filter((l) => l.get("layerId") == basemapId)[0];
      const otherBasemaps = layerGroup
        .getLayersArray()
