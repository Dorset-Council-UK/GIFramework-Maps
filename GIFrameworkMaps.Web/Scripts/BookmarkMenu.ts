...
export class BookmarkMenu {
  ...
  private addListeners() {
    ...
    const addBookmarkForm: HTMLFormElement = document.querySelector(
    ...
    const dropdownMenuEle = document.querySelector(
      "#gifw-bookmarks-list",
    ).parentElement;
  ...
  private async getBookmarks() {
    const bookmarksListContainer = document.querySelector(
      "#gifw-bookmarks-list",
    );
    bookmarksListContainer
      .querySelectorAll("li.bookmark-list")
      ?.forEach((item) => {
        item.remove();
      });
    try {
      const resp = await fetch(
        `${document.location.protocol}//${this.gifwMapInstance.config.appRoot}API/Bookmarks`,
      );
      ...
      if (!resp.ok) {
        ...
        if (bookmarks && bookmarks.length !== 0) {
          this.renderBookmarks(
            bookmarks,
            bookmarksListContainer as HTMLElement,
          );
        } else {
          bookmarksListContainer.insertAdjacentHTML(
            "afterbegin",
            `<li class="bookmark-list"><span class="dropdown-item-text mb-3 text-center">No bookmarks saved yet</span></li>`,
          );
        }
      }
    } catch (e) {
      ...
      bookmarksListContainer.insertAdjacentHTML(
        "afterbegin",
        `<li class="bookmark-list"><span class="dropdown-item-text mb-3 text-center">There was a problem fetching your bookmarks</span></li>`,
      );
    }
  }
  ...
  private renderBookmarks(
    bookmarks: Bookmark[],
    bookmarksListContainer: HTMLElement,
  ) {
    const bookmarksListFragment = document.getElementById(
      "bookmarks-list-container-template",
    ) as HTMLTemplateElement;
    const bookmarksListInstance = document.importNode(
      bookmarksListFragment.content,
      true,
    );
    ...
    bookmarks.forEach((bookmark) => {
      const bookmarkfragment = document.getElementById(
        "bookmark-template",
      ) as HTMLTemplateElement;
      const bookmarkInstance = document.importNode(
        bookmarkfragment.content,
        true,
      );
      const bookmarkText = bookmarkInstance.querySelector(
        "a.dropdown-item",
      ...
      const bookmarkDeleteButton = bookmarkInstance.querySelector(
        "a.text-danger",
      ) as HTMLAnchorElement;
    ...
    bookmarksListContainer.insertAdjacentElement(
      "afterbegin",
      newBookmarkListItem,
    );
  }
  ...
  private openAddBookmarkModal() {
    ...
    const nameInput: HTMLInputElement =
      addBookmarkForm.querySelector('input[name="Name"]');
    const xInput: HTMLInputElement =
      addBookmarkForm.querySelector('input[name="X"]');
    const yInput: HTMLInputElement =
      addBookmarkForm.querySelector('input[name="Y"]');
    const zoomInput: HTMLInputElement =
      addBookmarkForm.querySelector('input[name="Zoom"]');
    const validationText: HTMLDivElement =
      addBookmarkForm.querySelector(".text-danger");
  ...
  private async addBookmark(formData: FormData) {
    ...
    const resp = await fetch(
      `${document.location.protocol}//${this.gifwMapInstance.config.appRoot}API/Bookmarks/Create`,
      {
        body: formData,
        method: "post",
      },
    ...
    if (resp.status == 201) {
      ...
      Alert.showTimedToast(
        "Success",
        '<span class="bi bi-check-circle text-success"></span> Bookmark added successfully',
  ...
  private async removeBookmark(bookmarkId: number): Promise<boolean> {
    if (confirm("Are you sure you want to delete this bookmark?")) {
      const response = await fetch(
        `${document.location.protocol}//${this.gifwMapInstance.config.appRoot}API/Bookmarks/Delete/${bookmarkId}`,
        { method: "DELETE" },
      );
      if (!response.ok) {
        Alert.showPopupError(
          "Something went wrong",
          "Something went wrong deleting your bookmark. Please try again later.",
        );
        return false;
      }
      this.getBookmarks();
  ...
  private hideBookmarkMenu(): void {
    ...
    const navbar = Collapse.getInstance(
      ".giframeworkMap > header > nav .navbar-collapse",
    );
    if (navbar) {
      navbar.hide();
    }
