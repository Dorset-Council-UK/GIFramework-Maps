...
export class GIFWMap {
    ...
    public initMap(): olMap {
        ...
        if (!locationProvidedByURL) { 
            const bounds: Extent = [this.config.bound.bottomLeftX, this.config.bound.bottomLeftY, this.config.bound.topRightX, this.config.bound.topRightY];
            const mapSize = map.getSize();
    ...
    `;
        const annotationStyleSidebar = new gifwSidebar.Sidebar('annotation-style-panel', 'Modify annotation style', 'Change the appearance of your annotations', `data:image/svg+xml; charset=utf8, ${encodeURIComponent(annotationStyleIcon)}`, 3, annotationStylePanel);
